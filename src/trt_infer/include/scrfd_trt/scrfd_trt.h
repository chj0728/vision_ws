...
class SCRFD_TRT {
    public:
        /** insightface：与 insightface Python SCRFD（letterbox 左上 + RGB）一致。
         *  namdvt：与 https://github.com/namdvt/SCRFD_FaceDetection_TensorRT scrfd_trt.cpp 一致（左/上补黑 + BGR）。
         *  README 里 Binding 顺序与 TensorRT 引擎内 tensor 枚举顺序可能不同，以 tensor 名为准。 */
        enum class Preprocess { InsightFacePython, NamdvtUpstream };

        ~SCRFD_TRT();
        SCRFD_TRT(std::string trt_path);
        void setPreprocess(Preprocess p);
    ...
    private:
        ...
        // CPU base anchors
        std::vector<float> base_anchors_8;
        std::vector<float> base_anchors_16;
        std::vector<float> base_anchors_32;
        ...
        // GPU buffers
        void* d_src_img;
        int max_src_img_size;
        float* d_base_anchors_8;
        float* d_base_anchors_16;
        float* d_base_anchors_32;
        void* d_out_proposals;
        int* d_out_count;
        const int MAX_PROPOSALS = 1000;
        ...
        void updateScorePolicy(const float* s8, size_t n8, const float* s16, size_t n16, const float* s32, size_t n32);

        Preprocess preprocess_{Preprocess::InsightFacePython};
        bool score_use_sigmoid_{false};
        bool score_policy_done_{false};
