...
TrtInferComponent::TrtInferComponent(const rclcpp::NodeOptions& options)
	...
	const std::string bbox_space = declare_parameter<std::string>("bbox_coord_space", "letterbox");
	const bool bbox_in_original_space = bbox_space == "original" || bbox_space == "orig";
	...
	engine_ = std::make_unique<YOLOEngine>(model_path_, conf_, bbox_in_original_space, engine_input_height,
										   engine_input_width);

	pub_ = create_publisher<trt_infer_msgs::msg::ScenePerceptionResult>(out_topic_, rclcpp::QoS(2).reliable());
	...
	if (depth_sync_exact_) {
		...
		sync_exact_->registerCallback(
			std::bind(&TrtInferComponent::onSyncedColorDepth, this, std::placeholders::_1, std::placeholders::_2));
	} else {
...
bool TrtInferComponent::decodeToFloatMeters(const Image::ConstSharedPtr& depth_msg, cv::Mat& depth_meters) {
...
void TrtInferComponent::onSyncedColorDepth(const Image::ConstSharedPtr& color_msg,
											const Image::ConstSharedPtr& depth_msg) {
	const auto start_time = std::chrono::steady_clock::now();
	if (!color_msg || !depth_msg) return;
	...
	if (is_jpeg) {
		...
		image = cv_bridge::toCvShare(color_msg, sensor_msgs::image_encodings::BGR8)->image;
	}
	ensureBgrU8C3(image);
	if (image.empty()) return;

	cv::Mat depth_meters;
	if (!decodeToFloatMeters(depth_msg, depth_meters)) return;

	const auto detections = engine_->inferWithDepth(
		image, depth_meters, conf_, min_depth_m_, max_depth_read_m_, static_cast<float>(depth_roi_y0_),
		static_cast<float>(depth_roi_y1_), static_cast<float>(depth_roi_x_margin_),
		static_cast<float>(depth_trim_close_ratio_), static_cast<float>(depth_percentile_));
	...
	for (size_t index = 0; index < detections.size(); ++index) {
		...
		if (only_human_class_ && detection.class_id != human_class_id_) continue;
		...
		if (depth_valid && distance_ema_enable_) {
			if (!distance_ema_valid_[index]) {
				...
				distance_ema_state_[index] = distance_ema_alpha_ * distance +
