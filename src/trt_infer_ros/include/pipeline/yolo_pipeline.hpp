...
private:
  ...
  double depth_trim_close_ratio_{0.0}; // 深度图像的裁剪比例，用于去除过近的深度值
  float distance_ema_alpha_{0.35f}; // 指数移动平均（EMA）滤波的平滑系数，用于距离值的平滑处理


  std::string bbox_space; // Bounding box coordinate space, can be "letterbox" or "original"
  bool bbox_in_original_space; // Whether the bounding box coordinates are in the original image space
  int engine_input_height; // Input height for the YOLO engine
  int engine_input_width; // Input width for the YOLO engine

  std::vector<float> distance_ema_state_; // 指数移动平均（EMA）滤波的状态，用于存储每个目标的平滑距离值
	std::vector<uint8_t> distance_ema_valid_; // 指数移动平均（EMA）滤波的有效性标志，用于指示每个目标的距离值是否有效
