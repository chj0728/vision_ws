...
| --- | --- | --- | --- | --- |
| 1 | `YOLOPipeline` | 彩色图、米制深度图 | 人体框、置信度、距离，创建 `persons` | 可选距离 EMA |
...
| `body_distance == -1` | 距离无效或未知；该人体仍在结果中。已估计距离超过 `max_distance_m` 的目标不进入人员列表 |
...
4. 只接受有限且在 `[min_depth_m, max_depth_read_m]` 内的深度；少于 3 个有效样本时不能得到有效距离。
...
模块先检查深度采样得到的距离：若 `distance > max_distance_m`，直接跳过该检测，不创建 `PersonMeta`。当前 `max_distance_m=5.0`：例如估计距离为 6 米的人不会输出人体框，也不会进入后续追踪匹配、人脸检测、头姿估计和身份识别；距离恰好为 5 米时仍保留。过滤发生在 EMA 平滑和距离取整之前，不会因历史平滑值偏小而保留当前超距目标。

距离无效或未知时仍保留人体，距离字段写为 `-1`，后续模块继续处理。这里判断的是采样得到的距离，不是无法观测的真实距离；例如深度样本全部超过 `max_depth_read_m` 后被采样层排除，结果属于距离未知，不能据此判定超距。

超距过滤不会省去 YOLO 推理和深度采样本身，只减少后续模块对该目标的处理。已有轨迹若因超距不再匹配，会按追踪器规则老化、保留或清理，而不是立即删除历史状态。
...
最后为每个保留检测创建 `PersonMeta`，写入人体框、置信度和距离，记录 `body_detection_ms`。`depth_scale_to_meters` 虽被加载，但未在该路径中使用；深度单位转换依赖 ROS 输入层。
...
| `max_distance_m` | 5.0 | 人员输出距离上限；估计距离超过时过滤整个目标，等于上限时保留 |
...
| 深度无效或未知 | 人体仍输出，距离为 -1，后续模块继续处理 |
| 估计距离小于、等于或超过 5 米 | 小于或等于上限时保留；超过上限时不输出人体，后续模块不处理该目标；开启 EMA 时仍按平滑前距离过滤 |
