...
| `body_distance == -1` | 深度无效或超过允许距离；该人体仍在结果中 |
...
模块用 `distance > 0 && distance <= max_distance_m` 判断距离是否有效。当前 `max_distance_m=5.0`：例如检测到 6 米处的人，仍输出人体框，只把距离写为 `-1`，追踪、人脸检测和识别仍可能继续。
...
| `max_distance_m` | 5.0 | 输出有效距离上限，超出时写 -1 |
...
| 深度无效或超过 5 米 | 人体仍输出，距离为 -1，人脸链路不因距离无效直接停止 |
