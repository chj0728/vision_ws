...
class ArcFaceTRT {
public:
    explicit ArcFaceTRT(const std::string& engine_path, float match_threshold = 0.40f);
    ~ArcFaceTRT();
     ...
     *  Uses the InsightFace/ArcFace standard template.  Returns empty Mat on failure. */
    ...
    bool extractEmbedding(const cv::Mat& aligned_112, FaceEmbedding& out_emb) const;

    // ── Utility ────────────────────────────────────────────────────────────────
    /** L2-normalise a 512-dim vector in-place. */
    static void l2Normalize(FaceEmbedding& emb);
...
private:
    ...
    void* d_input_{nullptr};   // GPU: 1×3×112×112 float32
    void* d_output_{nullptr};  // GPU: 1×512    float32
